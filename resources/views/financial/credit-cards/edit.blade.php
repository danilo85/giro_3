...
@section('title', 'Editar Cartão de Crédito - Giro')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Cartão de Crédito</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Atualize as informações do cartão {{ $creditCard->nome_cartao }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <form action="{{ route('financial.credit-cards.update', $creditCard) }}" method="POST" id="credit-card-form">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="nome_cartao" name="nome_cartao" value="{{ old('nome_cartao', $creditCard->nome_cartao) }}" required
                        ...
                        <div>
                            ...
                            <select id="bandeira" name="bandeira" required
                                ...
                                <option value="Visa" {{ old('bandeira', $creditCard->bandeira) == 'Visa' ? 'selected' : '' }}>Visa</option>
                                <option value="Mastercard" {{ old('bandeira', $creditCard->bandeira) == 'Mastercard' ? 'selected' : '' }}>Mastercard</option>
                                <option value="American Express" {{ old('bandeira', $creditCard->bandeira) == 'American Express' ? 'selected' : '' }}>American Express</option>
                                <option value="Elo" {{ old('bandeira', $creditCard->bandeira) == 'Elo' ? 'selected' : '' }}>Elo</option>
                                <option value="Hipercard" {{ old('bandeira', $creditCard->bandeira) == 'Hipercard' ? 'selected' : '' }}>Hipercard</option>
                                <option value="Diners" {{ old('bandeira', $creditCard->bandeira) == 'Diners' ? 'selected' : '' }}>Diners</option>
                                <option value="Discover" {{ old('bandeira', $creditCard->bandeira) == 'Discover' ? 'selected' : '' }}>Discover</option>
                                <option value="JCB" {{ old('bandeira', $creditCard->bandeira) == 'JCB' ? 'selected' : '' }}>JCB</option>
                                <option value="Outros" {{ old('bandeira', $creditCard->bandeira) == 'Outros' ? 'selected' : '' }}>Outros</option>
                            </select>
                            @error('bandeira')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="numero" name="numero" value="{{ old('numero', $creditCard->numero) }}" maxlength="19"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="limite_total" name="limite_total" value="{{ old('limite_total', number_format($creditCard->limite_total, 2, ',', '.')) }}" required
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 dark:text-gray-400">R$</span>
                                <input type="text" id="limite_utilizado" name="limite_utilizado" value="{{ old('limite_utilizado', number_format($creditCard->limite_utilizado ?? 0, 2, ',', '.')) }}"
                                    class="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-colors"
                                    placeholder="0,00">
                            </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="data_vencimento" name="data_vencimento" required
                                ...
                                @for($i = 1; $i <= 31; $i++)
                                    <option value="{{ $i }}" {{ old('data_vencimento', $creditCard->data_vencimento) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select id="data_fechamento" name="data_fechamento" required
                                ...
                                <option value="">Selecione o dia</option>
                                @for($i = 1; $i <= 31; $i++)
                                    <option value="{{ $i }}" {{ old('data_fechamento', $creditCard->data_fechamento) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="observacoes" name="observacoes" rows="4"
                                ...
                                placeholder="Informações adicionais sobre o cartão...">{{ old('observacoes', $creditCard->observacoes) }}</textarea>
                            @error('observacoes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <div class="flex items-center">
                                <input type="checkbox" id="ativo" name="ativo" value="1" {{ old('ativo', $creditCard->ativo) ? 'checked' : '' }}
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 border-t border-gray-200 dark:border-gray-600 rounded-b-xl">
                <div class="flex flex-col sm:flex-row items-center justify-center sm:justify-end gap-3">
                    <a href="{{ route('financial.credit-cards.index') }}" 
                       class="w-full sm:w-auto px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-center">
                        <i class="fas fa-times mr-2"></i>Cancelar
                    </a>
                    <button type="submit" 
                            class="w-full sm:w-auto px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Função para limpar valor monetário (remove formatação)
    ...
    // Função para verificar se o valor já está formatado corretamente
    function isAlreadyFormatted(value) {
        // Verifica se contém pontos como separadores de milhares e vírgula como decimal
    ...
    function formatCurrency(value) {
        // Limpa o valor primeiro
        value = cleanCurrencyValue(value);
        
        // Se está vazio, retorna vazio
        if (!value) return '';
        
        // Se já está formatado corretamente, não reformata
    ...
    function applyCurrencyMask(element) {
        ...
        // Se o valor está vazio ou é zero, não aplica formatação
        ...
        // Aplica a formatação apenas se necessário
        ...
        if (formatted !== value) {
            ...
            // Garante que a posição do cursor seja válida
    ...
    const inputs = ['nome_cartao', 'bandeira', 'data_vencimento', 'data_fechamento'];
    ...
    function updatePreview() {
        const nome = document.getElementById('nome_cartao').value || 'Nome do Cartão';
    ...
    document.getElementById('credit-card-form').addEventListener('submit', function(e) {
        // Prepare currency values for submission
