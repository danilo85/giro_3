...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Nova Transação</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                <form id="transaction-form" action="{{ route('financial.transactions.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative">
                            <input type="radio" id="income-radio" name="type" value="income" class="sr-only" {{ request('type') === 'income' ? 'checked' : '' }}>
                        ...
                        <label class="relative">
                            <input type="radio" id="expense-radio" name="type" value="expense" class="sr-only" {{ request('type') === 'expense' ? 'checked' : '' }}>
                ...
                <!-- Valor -->
                <div class="mb-4">
                    <label for="amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Valor *</label>
                    <div class="relative">
                ...
                <div class="mb-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <label class="relative">
                            <input type="radio" name="status" value="paid" class="sr-only">
                ...
                <div class="mb-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                        ...
                        <label class="flex items-center mb-3">
                            <input type="checkbox" id="is_installment" name="is_installment" class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
                        ...
                        <div id="installment-options" class="space-y-3 hidden">
                            <div>
                                ...
                                <input type="number" id="installments" name="installments" min="2" max="60" value="2"
                                       class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                            <div>
                                <label for="installment_value" class="block text-xs text-gray-600 dark:text-gray-400 mb-1">Valor por parcela</label>
                                <input type="text" id="installment_value" name="installment_value" readonly
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                        ...
                        <label class="flex items-center mb-3">
                            <input type="checkbox" id="is_recurring" name="is_recurring" class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="Informações adicionais sobre esta transação..."></textarea>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                        <div id="file-upload-container" data-transaction-id="new">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Preencher data de vencimento com hoje
...
function setupEventListeners() {
    ...
    const fieldsToListen = [
        'description', 'amount', 'due_date', 'category_id', 
    ...
    const installmentsInput = document.getElementById('installments');
    const amountInput = document.getElementById('amount');
    if (installmentsInput && amountInput) {
        function calculateInstallmentValue() {
            const totalAmount = amountInput.value.replace(/[^\d,]/g, '').replace(',', '.');
...
function updatePreview() {
    ...
    if (previewType) {
        if (typeChecked) {
            const isIncome = typeChecked.value === 'income';
    ...
    if (previewStatus) {
        previewStatus.textContent = statusChecked ? (statusChecked.value === 'paid' ? 'Pago' : 'Pendente') : 'Não definido';
    }
}
...
function handleFormSubmit(event) {
    ...
    // Mapear tipo: income -> receita, expense -> despesa
    const mappedType = typeValue === 'income' ? 'receita' : typeValue === 'expense' ? 'despesa' : typeValue;
    
    // Mapear status: pending -> pendente, paid -> pago
    const mappedStatus = statusValue === 'pending' ? 'pendente' : statusValue === 'paid' ? 'pago' : statusValue;
    
    console.log('Debug - Tipo original:', typeValue, '-> Tipo mapeado:', mappedType);
    console.log('Debug - Status original:', statusValue, '-> Status mapeado:', mappedStatus);

    // Verificar se parcelamento está marcado
    ...
    // Obter o tipo de recorrência selecionado
    ...
    const formData = {
        ...
        credit_card_id: document.getElementById('credit_card_id').value || null,
        data: document.getElementById('due_date').value,
        status: mappedStatus,
    ...
    // Adicionar installment_count APENAS se parcelamento estiver marcado
    if (isInstallmentChecked) {
        formData.installment_count = document.getElementById('installments')?.value || 2;
    }
    ...
    .then(async (data) => {
        if (data.success || data.message) {
            showToast(data.message || 'Transação criada com sucesso!', 'success');
            
            // Mover arquivos temporários para a transação criada
            if (data.transaction && data.transaction.id && window.moveTemporaryFilesToTransaction) {
                try {
                    await window.moveTemporaryFilesToTransaction(data.transaction.id);
                    console.log('Arquivos temporários movidos com sucesso');
                } catch (error) {
                    console.error('Erro ao mover arquivos temporários:', error);
                    // Não bloquear o fluxo se houver erro com arquivos
                }
            }
...
function validateForm() {
    ...
    const cardId = document.getElementById('credit_card_id').value;
    const dueDate = document.getElementById('due_date').value;
