...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="mb-6">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Editar: {{ $partner->name }}</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Parceiro</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Atualize as informações do parceiro {{ $partner->name }}</p>
            </div>
            <div class="flex items-center space-x-3">
                <a href="{{ route('partners.show', $partner) }}" class="inline-flex items-center justify-center w-10 h-10 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Visualizar">
    ...
    <div class="max-w-7xl mx-auto">
        <form action="{{ route('partners.update', $partner) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="description" 
                                      ...
                                      placeholder="Descreva o parceiro (opcional)">{{ old('description', $partner->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    @if($partner->logo_path)
                        <div class="mb-6">
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="h-20 w-20 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        Logo atual do parceiro. Você pode fazer upload de uma nova imagem para substituí-la.
                                    </p>
                                    <button type="button" 
                                            onclick="removeLogo()" 
                                            class="mt-2 inline-flex items-center px-3 py-1 bg-red-100 text-red-800 text-xs font-medium rounded-full hover:bg-red-200 dark:bg-red-900 dark:text-red-200 dark:hover:bg-red-800 transition-colors">
                            ...
                            <label for="logo" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 transition-colors">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6" id="upload-placeholder">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        @if($partner->logo_path)
                                            PNG ou JPG (MAX. 2MB) - Substituirá o logo atual
                                        @else
                                            PNG ou JPG (MAX. 2MB)
                                        @endif
                                    </p>
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="mt-2 text-sm text-blue-700 dark:text-blue-300">
                                        <ul class="list-disc list-inside space-y-1">
                                            ...
                                            @if($partner->logo_path)
                                                <li><strong>Deixe em branco para manter o logo atual</strong></li>
                                            @endif
                                        </ul>
                                    </div>
                                </div>
            ...
            <!-- Campo oculto para remoção de logo -->
...
function removeLogo() {
    if (confirm('Tem certeza que deseja remover o logo atual? Esta ação não pode ser desfeita.')) {
        document.getElementById('remove_logo').value = '1';
        
        // Esconder a seção do logo atual
        const currentLogoSection = document.querySelector('.mb-6');
        if (currentLogoSection) {
            currentLogoSection.style.display = 'none';
        }
        
        // Atualizar o texto do placeholder
        const placeholderText = document.querySelector('#upload-placeholder p:last-child');
        if (placeholderText) {
            placeholderText.textContent = 'PNG ou JPG (MAX. 2MB) - Logo atual será removido';
        }
    }
}
</script>
